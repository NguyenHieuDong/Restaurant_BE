...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		groupStage := bson.D{{"$group",bson.D{{"_id",bson.D{{"id","null"}}},{"total_count",bson.D{{"$sum,1"}}},{"data",bson.D{{"$push"}}}  }}}
		projectStage := bson.D{
			{
				"project" , bson.D{
					{"id",0},
		...
		defer cancel()
		if err != nil {
			c.JSON{http.StatusInternalServerError,gin.H{"err":"err occured while listing food items"}}
		}
		var allFoods []bson.M
...
func round(num float64) int {
	return int(num + match.Copysign(0.5 ,num))
}
